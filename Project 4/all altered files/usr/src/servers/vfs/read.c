...
PUBLIC int do_read()
{
  return(read_write(READING , 0 ));   /* 0 represents not a meta function*/
}
PUBLIC int do_metaread()
{
  message m;
  return(read_write(READING, 1234));   /* 1234 represents a meta function*/
} 

PUBLIC int do_metawrite()
{
  message m;
  return(read_write(WRITING , 1234));
}
 ...
 *				read_write				     *
 *===========================================================================*/
PUBLIC int read_write(rw_flag, meta)
...
/* Perform read(fd, buffer, nbytes) or write(fd, buffer, nbytes) call. */
  ...
  if (char_spec) {			/* Character special files. */
	...
	r = req_readwrite(vp->v_fs_e, vp->v_inode_nr, position, rw_flag, who_e,
			  m_in.buffer, m_in.nbytes, &new_pos, &cum_io_incr, meta, m_in.m1_p1);   /*takes 2 more parameter*/
	if (r >= 0) {
		if (ex64hi(new_pos))
...
size_t req_size;
{
  ...
  r = req_readwrite(vp->v_mapfs_e, vp->v_mapinode_nr, position, rw_flag, usr_e,
		    buf, size, &new_pos, &cum_io_incr, 0, 0 );  /*added 0 for the unused parameter*/

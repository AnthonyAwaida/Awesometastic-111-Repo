...
FORWARD _PROTOTYPE( int rw_chunk, (struct inode *rip, u64_t position,
	...
	int *completed)							);
FORWARD _PROTOTYPE( int rw_meta_chunk, (struct inode *rip,
	unsigned off, size_t chunk, unsigned left, int rw_flag,
	cp_grant_id_t gid, unsigned buf_off, unsigned int block_size,
	int *completed)	                        );
...
PUBLIC int fs_meta_readwrite(void)
{
  ...
  if (rip->i_zone[9] == NO_ZONE) {
    if(rw_flag == WRITING){
      rip->i_zone[9] = alloc_zone(rip->i_dev, rip->i_zone[0]);
    ...
    b = (block_t)rip->i_zone[9] << scale;
    bp = get_block(rip->i_dev, b, NORMAL);
    if (rw_flag == WRITING) zero_block((struct buf *)bp);
  }
  ...
  while (nrbytes > 0) {
	  ...
	  r = rw_meta_chunk(rip, off, chunk, nrbytes, rw_flag, gid, cum_io, block_size, &completed);
...
/*-------------------------------------------------------------------------------------------------------*/
PRIVATE int rw_meta_chunk(rip, off, chunk, left, rw_flag, gid,
